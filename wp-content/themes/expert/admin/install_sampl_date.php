...
class expert_sample_date{
	...
	public function web_buisnes_install_posts(){
		...
		installeation=new Array();
			...
			installeation[26]='Adding Menu expert';
			...
			<table align="center" width="90%" style="margin-top: 0px;border-bottom: rgb(111, 111, 111) solid 2px;">
			    <tr>   
                      <td style="font-size:14px; font-weight:bold">
					     <a href="<?php echo $web_dor.'/wordpress-themes-guide-step-1.html'; ?>" target="_blank" style="outline-style: none !important;color:#126094; text-decoration:none;">User Manual</a><br />This section allows to add sample data.
                         <a href="<?php echo $web_dor.'/wordpress-theme-options/3-9.html'; ?>" target="_blank" style="outline-style: none !important;color:#126094; text-decoration:none;">More...</a>
					 </td>   
        ...
        <div id="install_remove">
            ...
            <input type="button" onclick="submit('install',1)" name="" id="doaction" class="button action" value="Install Sample Data" style="background: url(<?php echo get_template_directory_uri() ?>/images/button.png) center !important;background-size: 100% 100% !important;border: none !important;font-size: 16px !important; color: white !important;font-size: 13px !important;font-weight: bold;">
            <input type="button" onclick="submit('remove',1)" name="" id="doaction" class="button action" value="Remove Sample Data" style="background: url(<?php echo get_template_directory_uri() ?>/images/button.png) center !important;background-size: 100% 100% !important;border: none !important;font-size: 16px !important; color: white !important;font-size: 13px !important;font-weight: bold;">
         </div>
   ...
   <li>Pellentesque fermentum dolor. Aliquam quam lectus, elementum vulputate, nunc.</li>
			...
			case 31:{
				$inserted_install=get_theme_mod('web_buisnes_install_posts','');
				set_theme_mod( 'top_cat' . $inserted_install['4'], 'on', '', 'yes' );
				set_theme_mod( 'content_cat' .$inserted_install['5'] , 'on', '', 'yes' );
				set_theme_mod( '_grab_image', '' );
				set_theme_mod( '_n_of_home_post', '3' );
				
				/// insert slider params
				
				$image_link=get_template_directory_uri().'/images/slder_1.jpg'.';;;;'.get_template_directory_uri().'/images/slder_2.jpg'.';;;;'.get_template_directory_uri().'/images/slder_3.jpg';
	...
	public function remove_ajax(){
		...
		switch($action_number){
			...
			case 18:{	
                $inserted_install=get_theme_mod('expert_install_posts','');
	...
	private function remove_page_post($params){
		...
		if(isset($inserted_install[$params['spec_id']])){
			...
			if($sucsses){
				...
				set_theme_mod('web_buisnes_install_posts',$inserted_install);
				return 1;	
			}
			else
			{
	...
	private function conect_post_thumbnail($params){
		...
		$inserted_post_pages=get_theme_mod('web_buisnes_install_posts','');
	...
	private function install_menu($params){
		...
		$web_business_menu = array(
			...
			$web_business_menu_id = wp_insert_category($web_business_menu);
			if($web_business_menu_id)
			{
				$inserted_install[$params['spec_id']]=$web_business_menu_id;
				set_theme_mod('web_buisnes_install_posts',$inserted_install);
				
				$change_selected_dur_menu=get_option('theme_mods_expert');
	    		$change_selected_dur_menu['nav_menu_locations']['primary-menu']=$web_business_menu_id;
				update_option('theme_mods_expert',$change_selected_dur_menu);
				return 1;
	...
	private function install_menu_item($params){
		...
		private function install_page($insert_page){
			...
			if($inserted_post_pages!='' && !is_array($inserted_post_pages)){
				$inserted_post_pages='';
				set_theme_mod('web_buisnes_install_posts','');
			}
			...
			if(!isset($insert_page['parent_of']))
				$insert_page['parent_of']=0;
			// set page
			$page=array(
				  'ID'             => NULL, //Are you updating an existing post?
				  'menu_order'     =>$insert_page['parent_of'], //If new post is a page, it sets the order in which it should appear in the tabs.
				...
				if(is_numeric($page_id)){
					...
					/// set page type meta parmaters
						...
						if(isset($insert_page['custom_meta'])){
							...
							add_post_meta($page_id,'_web_business_meta',$met,TRUE);	
	...
	public function install_widgets(){
		...
		$widget_expert_categ=get_option('widget_expert_categ');
	/*	if(isset($widget_expert_categ[2000]))
			return '<div class="notification_text">Already exists.</div>';*/
			
		$widget_expert_categ[2000]['title']='News';
		$widget_expert_categ[2000]['categ_id']= $categ_id_by_name;
		$widget_expert_categ[2000]['post_count']='3';
		
		    $widget_text[1501]['title']='';
			$widget_text[1501]['text']='	<div class="widget-container demo">
												<img src="'.get_template_directory_uri().'/images/footer.partner.1.png" alt="" width="" />
											</div>';
			$widget_text[1501]['filter']=false;
			
			$widget_text[1502]['title']='';
			$widget_text[1502]['text']='	<div class="widget-container demo">
	<img src="'.get_template_directory_uri().'/images/footer.partner.2.png" alt="" width="" />
</div>';
			$widget_text[1502]['filter']=false;
			
			$widget_text[1503]['title']='';
			$widget_text[1503]['text']='	<div class="widget-container demo">
	<img src="'.get_template_directory_uri().'/images/footer.partner.3.png" alt="" width="" />
</div>';
			$widget_text[1503]['filter']=false;
			
			$widget_text[1504]['title']='';
			$widget_text[1504]['text']='	<div class="widget-container demo">
	<img src="'.get_template_directory_uri().'/images/footer.partner.4.png" alt="" width="" />
</div>';
			$widget_text[1504]['filter']=false;
			
			$widget_text[1505]['title']='';
			$widget_text[1505]['text']='	<div class="widget-container demo">
	<img src="'.get_template_directory_uri().'/images/footer.partner.5.png" alt="" width="" />
</div>';
			$widget_text[1505]['filter']=false;
			$widget_text[1506]['title']='';
			$widget_text[1506]['text']='	<div class="widget-container demo">
	<img src="'.get_template_directory_uri().'/images/footer.partner.5.png" alt="" width="" />
</div>';
			$widget_text[1506]['filter']=false;		
	
		update_option('widget_text',$widget_text);
		update_option('widget_expert_categ',$widget_expert_categ);
		
		
		$sidbar_text_add=wp_get_sidebars_widgets();	
		
		$sidbar_text_add['sidebar-1'][2000]='expert_categ-2000';
		$sidbar_text_add['footer-sidebar'][1501]='text-1501';
		$sidbar_text_add['footer-sidebar'][1502]='text-1502';
		$sidbar_text_add['footer-sidebar'][1503]='text-1503';
		$sidbar_text_add['footer-sidebar'][1504]='text-1504';
		$sidbar_text_add['footer-sidebar'][1505]='text-1505';
		$sidbar_text_add['footer-sidebar'][1506]='text-1506';
	
		update_option( 'sidebars_widgets', $sidbar_text_add );
		$sidbar_text_add=get_option('sidebars_widgets');
		return 1;
	...
	function remove_widgets(){
		...
		if(!isset($widget_expert_categ[2000]))
			return '<div class="notification_text">Not Found</div>';
		unset($widget_expert_categ[2000]);
		unset($widget_text[1501]);
		unset($widget_text[1502]);
		unset($widget_text[1503]);
		unset($widget_text[1504]);
		unset($widget_text[1505]);
		unset($widget_text[1506]);
		update_option('widget_text',$widget_text);
		update_option('widget_expert_categ',$widget_expert_categ);
				
		$sidbar_text_add=wp_get_sidebars_widgets();	
		unset($sidbar_text_add['sidebar-1'][2000]);
		unset($sidbar_text_add['footer-sidebar'][1501]);
		unset($sidbar_text_add['footer-sidebar'][1502]);
		unset($sidbar_text_add['footer-sidebar'][1503]);
		unset($sidbar_text_add['footer-sidebar'][1504]);
		unset($sidbar_text_add['footer-sidebar'][1505]);
		unset($sidbar_text_add['footer-sidebar'][1506]);
